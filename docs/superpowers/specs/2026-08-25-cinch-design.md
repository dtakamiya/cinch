...
| 更新方式 | バッチ読み取り。サーバ側はファイル監視をしない。一覧表示中はフロントが 30 秒間隔で `/api/sessions` を静かに再取得（タブ非表示中は停止、「自動更新」トグルで切替）。手動再スキャンも可 |
...
- サーバ側のファイル監視による push 更新（SSE / WebSocket 配信）。現状は一覧表示中のフロントからの定期ポーリングで代替
- LLM による会話内容の定性評価
...
> **注記（2026-08-27 更新）**: 初版は 3 カテゴリ・計 9 ルールで設計した。以降、実データを見ながら
> ルールを追加し、現在は **計 15 ルール**。カテゴリ配点（35 / 35 / 30）と 100 点満点は不変。
> 個別ルールの最新の配点は `README.md` と `server/config/thresholds.ts` の `WEIGHTS` を正とする
> （このドキュメントは設計判断の記録であり、配点の一次ソースではない）。

3 カテゴリ、100 点満点。カテゴリごとの小計も算出し、「コストは良いが生産性が低い」といった診断を可能にする。

### コスト効率（35 点）

| ID | 配点 | 何を見るか | 満点条件 / 減点 |
|----|------|-----------|----------------|
| `cache-efficiency` | 12 | `cache_read / (cache_read + input + cache_creation)` | ヒット率が高いほど高得点。低い＝毎ターン文脈を再送している |
| `cache-ttl-waste` | 10 | 1h キャッシュ作成後、TTL 内に再利用されず失効した回数 | 失効 0 で満点。1h 作成は 5m 比で割高なので、使い切れないなら損 |
| `model-fit` | 8 | 単純作業（Read/Grep/Bash のみで完結したターン）に高コストモデルを使った割合 | 低いほど高得点 |
| `context-window-headroom` | 5 | 1 ターンで送られた文脈の最大トークン数が上限（200k 基準）に迫っているか | 上限の 50% 未満で満点、95% 以上で 0 点。/clear・/compact の目安 |

### 生産性（35 点）

| ID | 配点 | 何を見るか | 満点条件 / 減点 |
|----|------|-----------|----------------|
| `tool-error-rate` | 9 | `tool_result.is_error` の全ツール呼び出しに対する比率 | 5% 未満で満点、25% 超で 0 点 |
| `redundant-file-reads` | 6 | 同一ファイルを編集を挟まず 2 回以上 Read した回数 | 0 回で満点。読み直し＝文脈を捨てている |
| `parallel-tool-use` | 6 | 独立したツール呼び出しが 1 メッセージにまとめられている割合 | 逐次実行が多いと減点。往復回数がそのまま待ち時間になる |
| `turn-efficiency` | 7 | 1 assistant ターンあたりの出力トークン数 | 300 以上で満点、60 以下で 0 点。少ない＝往復だけ増えて生産量が乏しい |
| `oversized-tool-results` | 4 | 1 回で 12KB を超える tool_result の回数 | 0 回で満点。巨大な結果は以降のターンで文脈に載り続ける |
| `bash-over-native-tools` | 3 | 専用ツール（Read/Grep/Glob）で代替できる Bash（cat/grep/find 等）の割合 | 5% 未満で満点、35% 以上で 0 点 |

### ベストプラクティス遵守（30 点）

| ID | 配点 | 何を見るか | 満点条件 / 減点 |
|----|------|-----------|----------------|
| `subagent-delegation` | 7 | 大量出力を伴う探索をメインで直接行っているか、`isSidechain` に委譲しているか | 委譲されていれば加点 |
| `context-growth` | 8 | セッション中の `cache_read + input` の増加カーブ | 1 ターンあたりの平均増加が急なら減点。肥大を放置している |
| `claude-md-present` | 4 | セッションの `cwd` に CLAUDE.md が存在するか | 存在すれば満点 |
| `task-planning` | 4 | 一定規模以上（15 ターン・20 ツール呼び出し以上）のセッションで TodoWrite を使ったか | 1 回でも使えば満点。未使用で 0 点 |
| `verification-gap` | 7 | ファイル変更（3 回以上）に対する検証コマンド（test/build/typecheck 等）の実行比率 | 編集 1 件あたり 0.5 回以上で満点、0 回で 0 点 |
...
この方針の帰結として、以下の検出漏れ・推定誤差を許容している（いずれも意図的）:

- **`parallel-tool-use` はターン内の機会しか見ない。** 判定対象は「1 ターンで読み取り専用ツールを 1 件だけ呼んだ」ケース（`metrics.ts` の `parallelizableSequences`）。ターン A で Read、ターン B で Read のようにターンをまたいで分割された呼び出しは機会に数えない。連続する読み取りターンを 1 ターンに結合すべきだったかは、間に挟まる判断の有無がログに出ないため断定できない。
- **`cache-ttl-waste` の失効はイベントの時間的近接からの推定。** 1h キャッシュ作成（`cacheCreate1h > 0`）を pending キューに積み、後続ターンで `cacheRead > 0` かつ作成から 1 時間以内なら 1 件回収したものとみなす（`metrics.ts`）。tool_use と違い、どの `cache_read` がどの作成分に対応するかの ID がログに無いための近似。TTL 境界付近のタイミングや、複数の 1h 作成が並行するセッションでは実失効数とずれうる。数値がノイジーだと感じたら `thresholds.ts` の `cacheTtlWaste.minCreations` を上げてサンプルの少ないセッションを判定対象外にする。
